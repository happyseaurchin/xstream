...
## Design Decisions: What to Preserve

These decisions should carry forward to future attempts:

### 1. Coordinates as TEXT, not JSONB
```sql
t TEXT NOT NULL,  -- "348.1"
s TEXT NOT NULL,  -- "13.4"
i TEXT NOT NULL,  -- "21."
```
**Why:** Prefix matching (`LIKE '13%'`) is the core operation. TEXT with `text_pattern_ops` indexes enables this directly. JSONB would require extraction before comparison.

### 2. 0.x for Meta-Layer
Skills, rules, and system instructions live at sub-unity coordinates. This is Target G's foundation.
- `0.31` = Soft-LLM
- `0.32` = Medium-LLM
- `0.33` = Hard-LLM
- `0.1x` = Interface
...
**Why:** If code is content, it must have coordinates. 0.x is the natural place — below the magnitude point, in the meta-layer.

### 3. No Type Fields
The coordinate IS the type. Don't add `entity_type`, `content_type`, `category`, etc.

**Why:** Every type field is a category. Categories are the old way. Pscale replaces categories with coordinates.

### 4. Skills Loaded by Proximity
Edge functions query `WHERE s LIKE '0.3%'` to load skills. They don't have hard-coded prompts.

**Why:** Self-modification. Edit the text at `0.31`, and Soft-LLM behavior changes.

---

## Pitfalls to Avoid

Future attempts should NOT:

### 1. Add a `type` or `category` column
**Temptation:** "I need to distinguish skills from content from characters"
**Solution:** Coordinate distinguishes them. `S:"0.31"` is a skill. `S:"13."` is content. `I:"21."` is a character's identity anchor.

### 2. Create separate tables for skills/frames/characters
**Temptation:** "It's cleaner to have a skills table"
**Solution:** Skills are content at 0.x coordinates. One table.

### 3. Hard-code LLM prompts in edge functions
**Temptation:** "I'll just write the prompt in the function for now"
**Solution:** Prompts are content at 0.x. Load them dynamically. This is how Target G works.

### 4. Store frames
**Temptation:** "I need to know what content is visible to this user"
**Solution:** Frame = proximity query. `SELECT ... WHERE s LIKE prefix`. Computed fresh each time.

### 5. Add cosmology_id
**Temptation:** "Different fictional worlds need different content"
**Solution:** Cosmology is a high-pscale S prefix. `S:"1..."` = one world. `S:"2..."` = another. Same table.

### 6. Use JSONB for coordinates
**Temptation:** "`{t, s, i}` as a single JSONB field is cleaner"
**Solution:** Prefix matching on JSONB is inefficient. Three TEXT columns with indexes.
